...
//|  Mode: Stack LIMIT order tanpa TP/SL individual                  |
//|  Pilih sinyal: Mentor Mode (rule-based) atau AI Utama (ensemble) |
//|  Fitur baru: Maks Posisi + Basket TP                             |
//+------------------------------------------------------------------+
#property copyright "GoldRadar.ai"
#property version   "4.00"
#property description "Pujya EA 4 — Mentor/AI Utama | Maks Posisi | Basket TP"
...
enum SIGNAL_MODE
  {
   MODE_MENTOR   = 0, // Mentor Mode    — rule-based, selalu aktif, tidak butuh EA Mode admin
   MODE_AI_UTAMA = 1, // AI Utama       — ensemble AI, butuh EA Mode aktif di admin
  };

//--- Sensitivitas khusus Mentor Mode
enum SENSITIVITY
  {
   SENS_SUPER_AGGRESSIVE = 0, // Super Agresif — entry sangat mudah
   SENS_AGGRESSIVE       = 1, // Agresif
   SENS_NORMAL           = 2, // Normal (direkomendasikan)
   SENS_CONSERVATIVE     = 3, // Konservatif — entry hanya sinyal kuat
  };

//--- Input Parameters
input group "=== Koneksi API ==="
input string InpApiUrl   = "";   // URL API (dari halaman admin)
input string InpEaApiKey = "";   // EA API Key (hanya diperlukan untuk AI Utama)

input group "=== Mode Sinyal ==="
input SIGNAL_MODE InpSignalMode  = MODE_MENTOR;          // Sumber sinyal
input SENSITIVITY InpSensitivity = SENS_NORMAL;          // Sensitivitas (Mentor Mode saja)
...
input double InpPriceChangeTrigger = 0.5;  // Buka order baru jika harga bergeser >= $X (0 = mati)

input group "=== Maks Posisi ==="
input int    InpMaxPositions = 5;    // Maks total posisi + pending (0 = tidak ada batas)

input group "=== Basket TP ==="
input double InpBasketTP    = 50.0;  // Tutup semua posisi jika floating P&L >= $X (0 = nonaktif)
input bool   InpBasketReset = true;  // Izinkan stack baru mulai lagi setelah Basket TP kena
...
int OnInit()
{
   ...
   // Label mode aktif
   if(InpSignalMode == MODE_MENTOR)
   {
      string sens = SensitivityString();
      g_modeLabel = "MENTOR (" + sens + ")";
   }
   else
   {
      g_modeLabel = "AI UTAMA (Ensemble)";
   }
...
void OnDeinit(const int reason)
{
   ...
   Print("Pujya EA 4 dihentikan. Mode: ", g_modeLabel);
}
...
//+------------------------------------------------------------------+
//  Ambil string sensitivitas sesuai enum
//+------------------------------------------------------------------+
string SensitivityString()
{
   switch(InpSensitivity)
   {
      case SENS_SUPER_AGGRESSIVE: return "super_aggressive";
      case SENS_AGGRESSIVE:       return "aggressive";
      case SENS_CONSERVATIVE:     return "conservative";
      default:                    return "normal";
   }
}

//+------------------------------------------------------------------+
//  Ekstrak nilai string dari JSON response sederhana
//  Contoh: JsonExtract("{\"command\":\"BUY\",...}", "command") → "BUY"
//+------------------------------------------------------------------+
...
void CheckBasketTP()
{
   ...
   if(pnl >= InpBasketTP)
   {
      Print("=== [Pujya EA 4] 🎯 BASKET TP KENA! P&L=$", DoubleToString(pnl, 2),
      ...
      if(InpShowPanel)
      {
         SetLabelText(LBL_BASKET,
            "🎯 BASKET HIT! +" + DoubleToString(pnl, 2) +
...
//+------------------------------------------------------------------+
//  FETCH — pilih endpoint sesuai mode
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//  FETCH MENTOR MODE — GET /api/xauusd/mentor-signal (JSON)
//  Tidak perlu EA API Key. Tidak perlu EA Mode aktif di admin.
//  Respon: {"command":"BUY"|"SHORT"|"HOLD", "confidence":0.72, "price":..., "minTP":..., "minSL":...}
//+------------------------------------------------------------------+
void FetchMentorSignal()
{
   string sens = SensitivityString();
   string url  = InpApiUrl + "/api/xauusd/mentor-signal?sensitivity=" + sens;
   ...
   if(code == -1)
   {
      ...
      Print("[ERROR Mentor] ", msg);
      if(InpShowPanel) SetLabelText(LBL_STATUS, "❌ " + msg, clrRed);
      return;
   }
   if(code != 200)
   {
      Print("[ERROR Mentor] HTTP ", code);
      if(InpShowPanel) SetLabelText(LBL_STATUS, "Mentor HTTP " + IntegerToString(code), clrRed);
      return;
   }

   string json = CharArrayToString(recvBuf);
   StringTrimRight(json);
   StringTrimLeft(json);

   // Ekstrak field dari JSON
   string cmdRaw  = JsonExtract(json, "command");   // "BUY" | "SHORT" | "HOLD"
   string priceS  = JsonExtract(json, "price");
   string confS   = JsonExtract(json, "confidence");
   string tpS     = JsonExtract(json, "minTP");
   string slS     = JsonExtract(json, "minSL");

   if(StringLen(cmdRaw) == 0)
   {
      Print("[ERROR Mentor] Respons tidak valid: ", StringSubstr(json, 0, 100));
      if(InpShowPanel) SetLabelText(LBL_STATUS, "Mentor: respons tidak valid", clrRed);
      return;
   }

   // Normalisasi: Mentor mengembalikan "SHORT" untuk sell
   g_rawCmd = cmdRaw;
   if(g_rawCmd == "SHORT") g_rawCmd = "SELL";

   g_price  = StringToDouble(priceS);
   g_conf   = StringToDouble(confS);

   Print("[Mentor] ", g_rawCmd,
...
//+------------------------------------------------------------------+
//  FETCH AI UTAMA — GET /api/xauusd/ea-signal?format=plain2 (plain text)
//  Butuh EA API Key. Butuh EA Mode aktif di admin (Mentor Mode widget).
//  Format: CMD|PRICE|TP1|TP2|TP3|SL|EL|EH|CONFIDENCE
//+------------------------------------------------------------------+
void FetchAiUtamaSignal()
{
   if(StringLen(InpEaApiKey) == 0)
   {
      Print("[ERROR AI Utama] EA API Key kosong — isi input InpEaApiKey");
      if(InpShowPanel) SetLabelText(LBL_STATUS, "❌ EA API Key kosong", clrRed);
      return;
   }
   ...
   if(code == -1)
   {
      ...
      string msg = (err == 5203)
         ? "Tambahkan URL di: Tools > Options > Expert Advisors > Allow WebRequest"
         : "Koneksi gagal (err=" + IntegerToString(err) + ")";
      Print("[ERROR AI Utama] ", msg);
      if(InpShowPanel) SetLabelText(LBL_STATUS, "❌ " + msg, clrRed);
      return;
   }
   if(code != 200)
   {
      Print("[ERROR AI Utama] HTTP ", code);
      if(InpShowPanel) SetLabelText(LBL_STATUS, "AI Utama HTTP " + IntegerToString(code), clrRed);
      return;
   }

   // Parse plain2: CMD|PRICE|TP1|TP2|TP3|SL|EL|EH|CONF
   ...
   if(numParts < 2)
   {
      Print("[ERROR AI Utama] Respons tidak valid: ", resp);
      if(InpShowPanel) SetLabelText(LBL_STATUS, "AI Utama: respons tidak valid", clrRed);
      return;
   }
...
//+------------------------------------------------------------------+
//  PROSES SINYAL — logika bersama untuk kedua mode
//+------------------------------------------------------------------+
void ProcessCommand()
{
   ...
   if(InpCooldownSec > 0 && g_lastOrderAt > 0)
   {
      ...
      {
         Print("[Pujya EA 4] ⏳ Cooldown: ", elapsed, "s / ", InpCooldownSec, "s.",
   ...
   if(dirChanged)
   {
      ...
      Print("[Pujya EA 4] 🔄 Harga bergeser: $", DoubleToString(g_lastExecPrice, 2),
            " → $", DoubleToString(g_price, 2),
            " ($", DoubleToString(MathAbs(g_price - g_lastExecPrice), 2), ") — stack baru");
...
//+------------------------------------------------------------------+
//  Tutup semua posisi terbuka & hapus semua pending order EA ini
//+------------------------------------------------------------------+
void CloseAllOwn()
{
   for(int idx = PositionsTotal() - 1; idx >= 0; idx--)
   {
      ...
      if(!g_trade.PositionClose(ticket))
         Print("[Pujya EA 4] ⚠ Gagal tutup #", ticket, ": ", g_trade.ResultComment());
   }
   ...
   Print("[Pujya EA 4] Semua ditutup. Sisa Open=", CountOwnPositions(),
         " Pending=", CountOwnPendingOrders());
}
...
bool OpenPendingOrder(string cmd)
{
   if(g_price <= 0)
   {
      Print("[Pujya EA 4] ❌ Harga tidak valid ($", DoubleToString(g_price, 2), ") — dibatalkan.");
      return false;
   }
   ...
   if(InpMaxPositions > 0)
   {
      ...
      {
         Print("[Pujya EA 4] 🚫 Maks posisi (", total, "/", InpMaxPositions, ") — order diblokir.");
         return false;
      }
   }
...
void CreatePanel()
{
   ...
   MakeLabel(LBL_TITLE,  x+8, y+5,   "PUJYA EA 4  —  GoldRadar.ai",        C'180,80,220', 9);
   MakeLabel(LBL_MODE,   x+8, y+19,  "Mode: " + g_modeLabel,               C'100,200,255', 8);
   MakeLabel(LBL_STATUS, x+8, y+33,  "Menghubungkan...",                    clrGray,        8);
   MakeLabel(LBL_CMD,    x+8, y+49,  "---",                                 clrGray,       18);
   MakeLabel(LBL_STACK,  x+8, y+83,  "Open: 0  Pending: 0  Maks: 0",      clrSilver,      8);
   MakeLabel(LBL_PNL,    x+8, y+97,  "Floating P/L: $0.00",                clrSilver,      8);
   MakeLabel(LBL_BASKET, x+8, y+111, "Basket TP: menunggu...",              clrGray,        8);
   MakeLabel(LBL_CONF,   x+8, y+125, "Conf: --- | Auto: ON",               clrSilver,      8);
   MakeLabel(LBL_LAST,   x+8, y+139, "Eksekusi terakhir: ---",             clrGray,        8);
   MakeLabel(LBL_TIME,   x+8, y+198, "---",                                 C'90,40,120',   7);
...
void UpdatePanelPnl()
{
   ...
   {
      double pct = pnl / InpBasketTP * 100.0;
      color  bc  = pnl >= InpBasketTP ? clrGold : (pct >= 80 ? clrYellow : clrSilver);
      SetLabelText(LBL_BASKET,
         "🎯 Basket TP: " + (pnl >= 0 ? "+" : "") + "$" + DoubleToString(pnl, 2) +
         " / $" + DoubleToString(InpBasketTP, 2) +
         " (" + DoubleToString(pct, 0) + "%)",
         bc);
   }
   else if(InpBasketTP > 0)
      SetLabelText(LBL_BASKET, "🎯 Basket TP: $" + DoubleToString(InpBasketTP, 2) + " | tidak ada posisi", clrGray);
   else
...
void UpdatePanel(string cmd)
{
   ...
   string modeStr = InpSignalMode == MODE_MENTOR ? "MENTOR" : "AI UTAMA";
   string maxStr  = InpMaxPositions > 0 ? " | Maks:" + IntegerToString(InpMaxPositions) : "";

   SetLabelText(LBL_MODE,   "Mode: " + g_modeLabel, C'100,200,255');
   SetLabelText(LBL_STATUS,
      "✅ Terhubung | " + modeStr + " | poll " + IntegerToString(POLL_INTERVAL_SEC) + "s" + maxStr,
      clrLime);
   ...
   if(g_lastExec != "HOLD" && g_lastOrderAt > 0)
      lastInfo = g_lastExec + " | $" + DoubleToString(g_lastExecPrice, 2)
...
{
   string labels[] = {
      LBL_BG, LBL_TITLE, LBL_MODE, LBL_STATUS, LBL_CMD,
