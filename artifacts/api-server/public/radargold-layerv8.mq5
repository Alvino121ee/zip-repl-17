...
// TP tidak lagi per-layer — semua layer pakai TP dari sinyal (g_tp = targetPrice prediksi AI M5)
...
double   g_price       = 0;
double   g_sl          = 0;
double   g_tp          = 0;   // TP dari sinyal — dipakai untuk SEMUA layer (bukan per-layer)
double   g_conf        = 0;
...
int OnInit()
{
   ...
   Print("Layers     : ", InpLayerCount, " | Gap: $", InpLayerGapUSD, " | TP: dari sinyal AI (g_tp)");
...
void StartNewSession(string cmd)
{
   ...
   for(int i = 1; i <= MAX_LAYERS; i++)
   {
      ...
      // Semua layer pakai TP yang sama dari sinyal AI (bukan per-layer dari entry masing-masing)
   ...
   if(g_layerState[1] == LAYER_OPEN)
   {
      ...
      Print("[Layer] === Sesi baru AKTIF: ", cmd, " | Harga=$", DoubleToString(price, 2),
            " | TP sinyal=$", DoubleToString(g_tp, 2),
...
void FetchAndProcess()
{
   ...
   if(numParts >= 6)
   {
      ...
      g_tp         = StringToDouble(parts[2]);   // TP1 dari sinyal (targetPrice prediksi AI)
   ...
   if(InpReverseMode)
   {
      ...
      if(cmd == "BUY" || cmd == "SELL")
      {
         ...
         double tpDist = (g_price > 0 && g_tp > 0) ? MathAbs(g_price - g_tp) : slDist * 1.5;
         g_sl = (cmd == "BUY")
            ? NormalizeDouble(g_price - slDist, _Digits)
            : NormalizeDouble(g_price + slDist, _Digits);
         g_tp = (cmd == "BUY")
            ? NormalizeDouble(g_price + tpDist, _Digits)
            : NormalizeDouble(g_price - tpDist, _Digits);
      }
   }
