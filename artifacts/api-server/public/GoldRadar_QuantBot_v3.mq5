...
{
   EventKillTimer();
   Print("[GoldRadar] EA dinonaktifkan.");
}
...
void OnTimer()
{
   ...
   if(code != 200) {
      Print("[GoldRadar] ⚠️ HTTP ", code, " — retry dalam ", _pollInterval, "s");
      return;
   }
...
//════════════════════════════════════════════════════════════════════
//  TICK — hanya untuk refresh symbol info
//════════════════════════════════════════════════════════════════════
void OnTick()
{
   if(_isReady) _sym.RefreshRates();
}
...
void ProcessSignal(const string &response)
{
   // Tokenize
   string parts[];
   int count = StringSplit(StringTrimRight(StringTrimLeft(response)), '|', parts);
   if(count < 6) {
      Print("[GoldRadar] ⚠️ Response tidak valid: '", response, "'");
      return;
   }
   ...
   if(confidence < MinConfidence) {
      Print("[GoldRadar] ⚡ Skip — confidence ",
            DoubleToString(confidence * 100, 1), "% < min ",
            DoubleToString(MinConfidence * 100, 0), "%");
      _lastSignalId = signalId;
      return;
   }
   ...
   // ── Validasi TP / SL ─────────────────────────────────────────
   if(tp == 0 || sl == 0) {
      Print("[GoldRadar] ⚠️ TP atau SL = 0 — sinyal dilewati");
      _lastSignalId = signalId;
      return;
   }

   // ── Tutup posisi berlawanan jika CloseOnReverse ───────────────
   if(CloseOnReverse && HasOpenPosition()) {
      ...
      if(isReverse) {
         Print("[GoldRadar] 🔄 Reverse → tutup ", (existing == POSITION_TYPE_BUY ? "BUY" : "SELL"));
         CloseAllPositions("Reverse ke " + command);
      }
   }

   // ── OneTradeOnly: skip jika ada posisi aktif ─────────────────
   if(OneTradeOnly && HasOpenPosition()) {
      if(VerboseLog)
         Print("[GoldRadar] ℹ️ Posisi masih aktif, OneTradeOnly=true → skip entry");
      _lastSignalId = signalId;
...
int MakeHttpGet(const string url, string &response)
{
   ...
   if(code == -1) {
      Print("[GoldRadar] WebRequest error: ", GetLastError(),
            " — Pastikan URL di-whitelist di Tools → Options → EA → WebRequest");
      return -1;
   }
